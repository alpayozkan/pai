...
class SWAGInference(object):
        ...
        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:
            ...
            for epoch in pbar:
                ...
                if epoch % self.swag_update_freq == 0:
                    self.n += 1 # epoch / self.swag_update_freq
                    self.update_swag()
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        # in order to cat along bma samples
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            var = self.w2_swa[name] - self.w_swa[name]**2
            print("var: {}".format(var))
            # var = torch.clamp(var, 1e-10)
            # print("var_clamp: {}".format(var))
            print("negval: {}".format(torch.sum(var<0)))
            current_std = torch.sqrt(var)
            print("std: {}".format(current_std))
