...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.running_first_moment = self._create_weight_copy()
        self.running_second_moment = self._create_weight_copy()
        self.n = 0
        ...
        self.dvec = self._create_weight_copy()
    ...
    def update_swag(self) -> None:
        ...
        for name, param in current_params.items():
            ...
            # TODO(DEL)
            self.running_first_moment[name] = (self.n * self.running_first_moment[name] + param.data)/(self.n + 1)
            self.running_second_moment[name] = (self.n * self.running_second_moment[name] + param.data*param.data)/(self.n + 1)
            
            self.dvec[name] = param - self.running_first_moment[name] # theta_i - theta_bar: deviation
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            if len(self.D) == self.deviation_matrix_max_rank:
                self.D.popleft() # drop K oldest param dict

            self.D.append(self.dvec) # add whole param dictionary: newest deviation stored
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.running_first_moment[name]
            var = self.running_second_moment[name] - self.running_first_moment[name]*self.running_first_moment[name]
            var = torch.clamp(var, 1e-10)
            current_std = torch.sqrt(var)
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                sampled_param = current_mean + current_std * z_1 * 1/torch.sqrt(torch.tensor(2))

                dv = []
                for D_i in self.D:
                    dv.append(D_i[name].unsqueeze(0))
                Dparam = torch.cat(dv)
